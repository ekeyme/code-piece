...
int Command_fetch(apr_pool_t *p, const char *url, int fetch_only)
{
	...
	if(apr_fnmatch(GIT_PAT, info.path, 0) == APR_SUCCESS) {
		...
		if(info.scheme) {
			rc = Shell_exec(CURL_SH, "URL", url, "TARGET", TAR_BZ2_PAT, NULL);
			check(rc == 0, "Curl failed.");
		}
		...
		const char *make_opts, const char *install_opts) 
{
	...
	check(access(BUILD_DIR, X_OK | R_OK | W_OK) == 0, 
			"Cannot access build directory: %s", BUILD_DIR);

	// actually do an install
	if(access(CONFIG_SCRIPT, X_OK) == 0) {
		log_info("A configure script is found, running it.");
		...
		const char *make_opts, const char *install_opts) 
{
	...
	check(rc == 0, "Failed to cleanup brefore building.");
